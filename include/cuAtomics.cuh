...
#ifndef CUATOMICS_CUH
#define CUATOMICS_CUH
...
//#if defined (CUDA) /*preprossing for the CUDA environment */
  ...
  __host__ __device__ static __inline__ cuFloatComplex cuR2Cf(float x, float y)
  {
  ...
  __host__ __device__ static __inline__ cuFloatComplex cuCCaddf(cuFloatComplex x, cuFloatComplex y)
  {
  ...
  __host__ __device__ static __inline__ float cuFFaddf(float x, float y)
  {
    float add;
    add = x + y;
    return add;
  ...
  __host__ __device__ static __inline__ float cuFFmulf(float x, float y)
  {
    float prod;
    prod = (x) * (y);
    return prod;
  ...
  __host__ __device__ static __inline__ float cuFFscalef(float x, float s)
  {
    float scale;
    scale = s * x;
    return scale;
  ...
  __host__ __device__ static __inline__ cuFloatComplex cuCCscalef(cuFloatComplex a, float s)
  {
  ...
  __host__ __device__ static __inline__ float cuReCf(cuFloatComplex a)
  {
    float realpart;
    realpart = a.x;
    return realpart;
  ...
  __host__ __device__ static __inline__ float cuImCf(cuFloatComplex a)
  {
    float imagpart;
    imagpart = a.y;
    return imagpart;
  ...
  __host__ __device__ static __inline__ float cuDDadd(double x, double y)
  {
    double add;
    add = x + y;
    return add;
  ...
  __host__ __device__ static __inline__ float cuFFsqrtf(float x, float y)
  {
    float sqrtprod;
    sqrtprod = sqrt( x * y );
    return sqrtprod;
  ...
  __host__ __device__ static __inline__ float cuReCCmulf(cuFloatComplex x,
							 cuFloatComplex y)
  {
    float prod;
    prod = cuCrealf(x) * cuCrealf(y) - cuCimagf(x) * cuCimagf(y);
  ...
  __host__ __device__ static __inline__ float cuReCCsmulf(cuFloatComplex x,
							  cuFloatComplex y)
  {
    float prod;
    prod = cuCrealf(x) * cuCrealf(y) + cuCimagf(x) * cuCimagf(y);
...
extern "C" {
  ...
  /********************************************************
   * Here add additional external access if needed
   */
