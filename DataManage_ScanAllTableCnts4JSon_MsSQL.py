...
if __name__ == '__main__':
    ...
    src.PythonCodes.DataManage_header.print_testcode_header(common=c, messageHandler=m)
    ...
    m.printMesgAddStr(" Connection        : ",c.getGreen(), "Database Server")
    m.printMesgAddStr(" Database          : ",c.getYellow(), queryMsSQL.credentialsMsSQL.database)
    m.printMesgAddStr(" Connecting        : ",c.getCyan(),"...")
    rc, cnxn = queryMsSQL.open_connection_database()
    ...
    while running:
        ...
        m.printMesgAddStr(" Database          : ",c.getYellow(), queryMsSQL.credentialsMsSQL.database)
        rc, full_table_list = queryMsSQL.query_GetFullTable_list(cursor)
        if rc != c.get_RC_SUCCESS(): exit(c.get_RC_FAIL())
        #print("full_table_list[:] --->: ", full_table_list[:])
        #---------------------------------------------------------------------------
        # Creating the Json file for the table counts...
        #---------------------------------------------------------------------------
        m.printMesg("Creating the Json file for the table counts")
        rc, table_list = queryMsSQL.generateTable_list(full_table_list)
        if rc != c.get_RC_SUCCESS(): exit(c.get_RC_FAIL())
        ...
        for i in range(len(table_list[:])):
            rc, table_count = queryMsSQL.query_GetTableCount(cursor, queryMsSQL.credentialsMsSQL.database, table_list[i])
