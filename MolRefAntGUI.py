...
Options:
  ...
  --cryo_em                   Cryo-EM main switch on gui
  --spectroscopy              Spectroscopy main switch on gui
...
##\brief Python3 method.
# DataManagerGuiApp application helper class
class MolRefAntGuiApp(object):
    def __init__(self,q):
        self.root = tk.Tk()
        self.root.geometry('500x250')
        self.root.title("MolRefAnt Application (Project Handler)")
        self.text_wid = tk.Text(self.root, height=200, width=200)
        self.text_wid.pack(expand=1,fill=tk.BOTH)
        self.root.after(200, self.CheckQueuePoll, q)
    #---------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# MolRefAnt_DBApp application
#-------------------------------------------------------------------------------
##\brief Python3 method.
# MolRefAnt_DBGuiApp application class
class MolRefAnt_DBGuiApp(object):
    """
    GUI Tkinter class for MolRefAnt_DB
    """
    ##\brief Python3 method.
    #MolRefAnt_DBGuiApp application class constructor
    def __init__(self, parent, c, m):
        ...
        #Starting the StateMachine
        self.m.printMesg("State Machine SimpleDevice class instantiated, log: "+ c.getLogfileName())
        self.device = src.PythonCodes.utils.StateMachine.simple_device.SimpleDevice(self.c, self.m)#SimpleDevice(c,m)
        ...
        # General Settings
        self.parent = parent
        self.parent.title("MolRefAnt-APP Data Handler v" + version)
        self.parent.option_add('*tearOff', False)
        ...
        self.helpMenu.add_command(label="Documentation",
                                  command=self.showDocumentation)
        self.helpMenu.add_command(label="About MolRefAnt-APP Data Handler",
                                  command=self.showAbout)
        ...
        #[Home]
        #[NeCEN]
        self.app_root         = tk.StringVar(value=os.getcwd())
        self.projectName      = tk.StringVar(value="")
        #/data/local/frederic/SourceDir_KriosData
        self.data_path        = tk.StringVar(value="E:\data\LC_MS")
        self.chromogram_file  = tk.StringVar(value="OC1000-3.raw")
        #self.targetDirectory  = tk.StringVar(value="/data/local/frederic/TargetDir")
        #Warp target directory
        self.WarpTargetDir    = tk.StringVar(value="/scratch/frederic/TargetDir/GatanScripts/WarpFolder")
        #Gain reference
        ...
        #Research Drive
        self.cdiName          = tk.StringVar(value="cdi-Frederic")
        self.resrchDrvSrcDir  = tk.StringVar(value="krios1/20210401_2001-997_EPU-SP_Fred")
        self.resrchDrvTrgDir  = tk.StringVar(value="/data/local/frederic/ResearchDriveDownload")
        # Nuage private could
        self.nuageDrvSrcDir  = tk.StringVar(value="Chromograms"+os.sep+"20210401_2001-997_EPU-SP_Fred")
        self.nuageDrvTrgDir  = tk.StringVar(value="E:\data\\Nuage")

        self.scan_number    = tk.IntVar(value="2885")     #Min resolution
        self.rt             = tk.StringVar(value="13.35")      #Max resolution
        ...
        #-----------------------------------------------------------------------
        #adding the frames to the main frame
        self.nb.add(self.splitframe, text='MolRefAnt_DB Project and Chromogram Input', underline=0, padding=10)
        #-----------------------------------------------------------------------
        # self.splitframe Frame
        #-----------------------------------------------------------------------
        #-----------------------------------------------------------------------
        ...
        # ROW 0
        row = 0
        ttk.Label(self.splitframe, text="Inputs", font = self.font_size_headers).grid(column=1, row=row, sticky=tk.W)
        ...
        ttk.Entry(self.splitframe, width=30, textvariable=self.app_root, foreground="green", font=self.font_size_all_text).grid(column=2, columnspan=10, row=row, sticky=(tk.W, tk.E))
        ttk.Button(self.splitframe, text="Set app path   ", style='DataManageAppStd.TButton', command=(lambda: self.load_directory(self.app_root))).grid(column=12, row=row, sticky=(tk.W,tk.E))

        # ROW 2
        row = 2
        ttk.Label(self.splitframe, text="Project Name (optional):", foreground="blue", font=self.font_size_all_text).grid(column=1, row=row, sticky=tk.E)
        ttk.Entry(self.splitframe, width=30, textvariable=self.projectName, foreground="green", font=self.font_size_all_text).grid(column=2, columnspan=10, row=row, sticky=(tk.W, tk.E))
        ttk.Button(self.splitframe, text="Set Project Name", style='DataManageAppStd.TButton', command=(lambda: self.projectNameEntryActivator(self.projectName))) .grid(column=12, row=row, sticky=(tk.W,tk.E))#tk.W)
        self.c.setProjectName(self.projectName.get())

        # ROW 4
        row = 4
        ttk.Label(self.splitframe, text="File to analyse:", foreground="blue", font=self.font_size_all_text).grid(column=1, row=row, sticky=tk.E)
        ttk.Entry(self.splitframe, width=30, textvariable=self.chromogram_file, foreground="green", font=self.font_size_all_text).grid(column=2, columnspan=10, row=row, sticky=(tk.W, tk.E))
        ttk.Button(self.splitframe, text="Set Source Directory", style='DataManageAppStd.TButton', command=(lambda: self.load_file( self.chromogram_file  ))).grid(column=12, row=row, sticky=(tk.W,tk.E))
        # ROW 5
        row = 5
        ttk.Label(self.splitframe, text="Source Directory:", foreground="blue", font=self.font_size_all_text).grid(column=1, row=row, sticky=tk.E)
        ttk.Entry(self.splitframe, width=30, textvariable=self.data_path, foreground="green", font=self.font_size_all_text).grid(column=2, columnspan=10, row=row, sticky=(tk.W, tk.E))
        ttk.Button(self.splitframe, text="Set Source Directory", style='DataManageAppStd.TButton', command=(lambda: self.load_directory(self.data_path))).grid(column=12, row=row, sticky=(tk.W,tk.E))

        # ROW 4
        #row = 4
        #ttk.Label(self.splitframe, text="File to analyse  :", foreground="blue").grid(column=1, row=4, sticky=tk.E)
        #ttk.Entry(self.splitframe, width=30, textvariable=self.gainRef, foreground="green").grid(column=2, columnspan=10, row=4, sticky=(tk.W, tk.E))
        #ttk.Button(self.splitframe, text="Load Gain References", command=(lambda: self.load_file(self.gainRef))).grid(column=12, row=4, sticky=tk.W)
        ...
        # ROW 8
        row = 8
        ttk.Label(self.splitframe, text="Chromogram Extension:", font=self.font_size_all_text).grid(column=1, row=8, sticky=tk.E)
        #Movie Extension
        chromoExt=["raw", "mgf", "txt", "xls", "cvs"]
        # Radiobutton callback function
        def radCall_ChromoExt():
            radSel_ChromoEXT=radVar_ChromoEXT.get()
            if   radSel_ChromoEXT == 0:
                self.chromo_ext = "raw"
                self.m.printMesgAddStr(" Chromogram ext = ", self.c.getYellow(), self.chromo_ext)
            elif radSel_ChromoEXT == 1:
                self.chromo_ext = "mgf"
                self.m.printMesgAddStr(" Chromogram ext = ", self.c.getMagenta(), self.chromo_ext)
            elif radSel_ChromoEXT == 2:
                self.chromo_ext = "txt"
                self.m.printMesgAddStr(" Chromogram ext = ", self.c.getWhite(), self.chromo_ext)
            elif radSel_ChromoEXT == 3:
                self.chromo_ext = "xls"
                self.m.printMesgAddStr(" Chromogram ext = ", self.c.getGreen(), self.chromo_ext)
            elif radSel_ChromoEXT == 4:
                self.chromo_ext = "cvs"
                self.m.printMesgAddStr(" Chromogram ext = ", self.c.getCyan(), self.chromo_ext)

        radVar_ChromoEXT = tk.IntVar()
        # Next we are selecting a non-existing index value for radVar_MovieEXT.
        radVar_ChromoEXT.set(97)

        # Now we are creating all three Radiobutton widgets within one loop.
        #print("row: ", row)
        for col in range(len(chromoExt)):
            set_col = col + 2
            #print("col = ", col)
            #print("chromoExt["+str(col)+"]", chromoExt[col])
            curRad = 'rad' + str(col)
            curRad = tk.Radiobutton(self.splitframe, text=chromoExt[col], variable=radVar_ChromoEXT, value=col, command=radCall_ChromoExt)
            curRad.grid(column=set_col, row=row, columnspan=4, sticky=tk.W)
        # ROW 9
        '''
        row = 9
        set_col = 0
        #print("row: ", row)
        # Now we are creating all three Radiobutton widgets within one loop.
        for col in range(2, len(chromoExt)): #len(chromoExt)
            set_col = col + 2
            #print("col = ", col)
            #print("chromoExt["+str(col)+"]", chromoExt[col])
            #curRad = 'rad' + str(col)
            curRad = tk.Radiobutton(self.splitframe, text=chromoExt[col], variable=radVar_ChromoEXT, value=col, command=radCall_ChromoExt)
            curRad.grid(column=set_col-2, row=row, columnspan=3, sticky=tk.W)
        '''
        #-----------------------------------------------------------------------
        # [processing Inputs]
        #-----------------------------------------------------------------------
        # ROW 10
        row = 10
        ttk.Label(self.splitframe, text="Processing Inputs", font=self.font_size_headers).grid(column=1, row=row, sticky=tk.W)

        # ROW 13
        row = 13
        #--scan_number=SCAN_NUMBER                [30.0] Minimum resolution
        text_width = 217
        ttk.Label(self.splitframe, text="Scan (int) [#]:", foreground="blue",font=self.font_size_all_text).grid(column=1, row=row, sticky=tk.E)
        ttk.Scale(self.splitframe, variable=self.scan_number, from_=1, to=5000, orient=tk.HORIZONTAL, length=text_width).grid(column=2, columnspan=2, row=row, sticky=tk.W)
        text_width = 6
        ttk.Entry(self.splitframe, textvariable=str(self.scan_number).split('.'[0]), foreground="green", width=text_width, font=self.font_size_all_text).grid(column=4, columnspan=1, row=row, sticky=tk.W)

        #--RT=RET_TIME                [0:25.0] from 0 to 25 minutes
        text_width = 190
        ttk.Label(self.splitframe, text="Retention Time [min]:", foreground="blue", font=self.font_size_all_text).grid(column=5, row=row, sticky=tk.E)
        ttk.Scale(self.splitframe, variable=self.rt, from_=0, to=25, orient=tk.HORIZONTAL, length=text_width).grid(column=6, columnspan=2, row=row, sticky=tk.W)
        text_width = 7
        ttk.Entry(self.splitframe, textvariable=self.rt, foreground="green", width=text_width, font=self.font_size_all_text).grid(column=8, columnspan=1, row=row, sticky=tk.W)
        ...
        '''
        #-----------------------------------------------------------------------
        # [Nuage data movement Inputs]
        #-----------------------------------------------------------------------
        # ROW 21
        row = 21
        ttk.Label(self.splitframe, text="Download Nuage Drive", font = self.font_size_headers).grid(column=1, row=row, sticky=tk.W)

        # ROW 22
        row = 22
        #cdi
        ttk.Label(self.splitframe, text="cdi Name:", foreground="blue", font=self.font_size_all_text).grid(column=1, row=row, sticky=tk.E)
        ttk.Entry(self.splitframe, width=30, textvariable=self.cdiName, foreground="green", font=self.font_size_all_text).grid(column=2, columnspan=10, row=row, sticky=(tk.W, tk.E))

        # ROW 23
        row = 23
        #name of directory on research drive
        ttk.Label(self.splitframe, text="Nuage Drive Source Directory:", foreground="blue", font=self.font_size_all_text).grid(column=1, row=row, sticky=tk.E)
        ttk.Entry(self.splitframe, width=30, textvariable=self.nuageDrvSrcDir, foreground="green", font=self.font_size_all_text).grid(column=2, columnspan=10, row=row, sticky=(tk.W, tk.E))
        ttk.Button(self.splitframe, text="Set Source Directory", style='DataManageAppStd.TButton', command=(lambda: self.load_directory(self.nuageDrvSrcDir))).grid(column=12, row=row, sticky=(tk.W,tk.E))
        # ROW 24
        row = 24
        #Target directory on system
        ttk.Label(self.splitframe, text="Download Target Directory:", foreground="blue", font=self.font_size_all_text).grid(column=1, row=row, sticky=tk.E)
        ttk.Entry(self.splitframe, width=30, textvariable=self.nuageDrvTrgDir, foreground="green", font=self.font_size_all_text).grid(column=2, columnspan=10, row=row, sticky=(tk.W, tk.E))
        ttk.Button(self.splitframe, text="Set Target Directory", style='DataManageAppStd.TButton', command=(lambda: self.load_directory(self.nuageDrvTrgDir))).grid(column=12, row=row, sticky=(tk.W,tk.E))
        ...
        row = 25
        # Copy/Move Fractions button
        ttk.Button(self.splitframe, text="Download From Nuage", style='GatanFramesGainApp.TButton', command=self.DownloadResearchDrive).grid(column=2, row=25, sticky=(tk.W,tk.E))


        # Stop All button
        ttk.Button(self.splitframe, text="Stop ALL", style='GatanFramesGainApp.TButton', command=self.stopAll).grid(column=5, row=row, sticky=(tk.W,tk.E))
        # Check Inputs and Launch DataManagerApp button
        ttk.Button(self.splitframe, text="Launch Pre-Proc", style='GatanFramesGainApp.TButton', command=self.checkInputsAndLaunchMolRefApp).grid(column=6, row=row, sticky=(tk.W,tk.E))
        ...
        # Stop Download Button
        ttk.Button(self.splitframe, text="Stop Download", style='GatanFramesGainApp.TButton', command=self.StopDownloading).grid(column=2, row=row, sticky=(tk.W,tk.E))

        # Rclone setup button
        ttk.Button(self.splitframe, text="Setup rclone", style='GatanFramesGainApp.TButton', command=self.setupRcloneConfig).grid(column=6, row=row, sticky=(tk.W,tk.E))
        ...
        # Binding upon return
        #-----------------------------------------------------------------------
        self.parent.bind("<Return>",self.checkInputsAndLaunchMolRefApp)
    #---------------------------------------------------------------------------
    # class methods
    #---------------------------------------------------------------------------
    #---------------------------------------------------------------------------
    ...
    # \param self     The self Object
    # \param *args    The list of arguments from the command prompt
    def checkInputsAndLaunchMolRefApp(self, *args):
        __func__= sys._getframe().f_code.co_name
        #-----------------------------------------------------------------------
        # Starting the statemachine
        #-----------------------------------------------------------------------
        ...
        os.chdir(self.app_root.get())
        #-----------------------------------------------------------------------
        # Populating the main object
        #-----------------------------------------------------------------------
        # Poulating the c object from the GUI input variables
        self.c.setSourcedir(self.data_path.get())
        self.c.setRAW_file(os.path.basename(self.chromogram_file.get()))
        self.c.setScan_number(self.scan_number.get())
        self.c.setRet_time(self.rt.get())
        #-----------------------------------------------------------------------
        # Creating the GUI object for the subprocess launch of DataManagerApp
        #-----------------------------------------------------------------------
        self.q2 = multiprocessing.Queue()
        self.q2.cancel_join_thread() # or else thread that puts data will not term
        gui = MolRefAntGuiApp(self.q2)
        #t9 = multiprocessing.Process(target=self.LaunchMolRefApp,
        #                             args=(self.c, self.m, self.q2,))
        #t9.start()
        #gui.root.mainloop()
        #t9.join()
        #print(" self.scan_number.get() ---> ", self.scan_number.get())
        # Launching the App
        self.LaunchMolRefApp(self.c, self.m, self.q2)
        #-----------------------------------------------------------------------
        # Starting the StateMachine
        #-----------------------------------------------------------------------
        #Enable scanning by setting the global flag to True.
    ...
    # \param m        The mnessdeanger class for output messaging and loggin
    # \param q2       The multiprocessing Queue object
    def LaunchMolRefApp(self, c, m, q2):
        __func__= sys._getframe().f_code.co_name
        q2.put("Launching MolRefApp App...\n")
        q2.put("Function         : "+__func__ + "\n")
        q2.put(" Source dir      : " + self.c.getSourcedir() + "\n")
        q2.put(" Chromogram file : " + self.c.getRAW_file() + "\n")
        q2.put(" Scan number     : " + str(self.c.getScan_number()) + "\n")
        q2.put(" Retention time  : " + str(self.c.getRet_time()) + "\n")
        #path_python = self.app_root.get()+os.sep+"src"+os.sep+"PythonCodes"+os.sep+"DataManage_preProcessing.py"
        m.printMesgAddStr(" Launching app   : ", c.getGreen(), "LaunchMolRefApp ...")
        m.printMesgAddStr(" Source dir      : ", c.getMagenta(), self.c.getSourcedir())
        m.printMesgAddStr(" Chromogram file : ", c.getYellow(), self.c.getRAW_file())
        m.printMesgAddStr(" Scan number     : ", c.getYellow(), self.c.getScan_number())
        m.printMesgAddStr(" Retention time  : ", c.getCyan(), self.c.getRet_time())
        msg = self.app_root.get() + os.sep + "MZMineMain.py " + \
              "--rawfile=" + self.c.getSourcedir() + os.sep + self.c.getYellow() + self.c.getRAW_file() + \
              self.c.getMagenta() + " --scan_number=" + self.c.getYellow() + str(self.c.getScan_number())
        m.printMesgAddStr(" Python launch ---> :", c.getMagenta(), msg)
        path_python = self.app_root.get() + os.sep + "MZMineMain.py " + \
    ...
    def showAbout(self):
        __func__= sys._getframe().f_code.co_name
        showinfo("About MolRefAnt-App Data Handler",
                 ("This is MolRefAnt-App Data Handler v."+\
                  version+".\n\n"+"This package is released under the Creative Commons"))
    #---------------------------------------------------------------------------
    # [showDocumentation] method for quiting the application
    #---------------------------------------------------------------------------
    ...
    def showDocumentation(self):
        __func__= sys._getframe().f_code.co_name
        showinfo("MolRefAnt Documentation",
                 "Please download the manual at https://gitlab.com/projects/..... To be added later")
#-------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    #instantiating messaging class
    logfile = c.getLogMolRefAnt_DB_GUI()
    m = src.PythonCodes.utils.messageHandler.messageHandler(logfile=logfile)
    # printing the header of Application
    src.PythonCodes.DataManage_header.print_MolRefAnt_DB_app_header(common=c, messageHandler=m)
    ...
    m.printCMesgCVal("MolRefAnt Version   : ", c.get_B_White(),version,
                     c.get_B_Green())
    ...
    if args['--noguiSplit'] == False:
        ...
        if os.path.isfile('target_*'):
            ...
            m.printMesgAddStr(" deleting       :", c.getYellow(),'target_* files')
        ...
        if args['--spectroscopy'] == True:
            ...
            molrefantapp = MolRefAnt_DBGuiApp(root, c=c, m=m)
            root.mainloop()
        ...
        if args['--noguiSplit'] == True:
            m.printMesg("Starting the MolRefAnt_DB_App GUI...")
            '''
           ...
           getFinalExit(c,m,rc)
           m.printCMesg("Invalid entry for MolRefAntGUI: MolRefAntGUI.py!!",c.get_B_Red())
#---------------------------------------------------------------------------
